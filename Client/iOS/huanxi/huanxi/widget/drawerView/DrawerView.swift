...
/*代码分析：
 ...
 功能:
 创建一个可以滑动打开和关闭的抽屉视图。
 提供多种抽屉状态（关闭、折叠、部分打开、完全打开）。
 支持自定义抽屉外观（圆角、阴影、背景颜色等）。
 支持手势操作，可以拖动抽屉来改变其状态。
 提供了丰富的委托方法，可以自定义抽屉的各种行为。
 主要类和结构体:
 DrawerPosition: 定义了抽屉的各种状态。
 Overlay: 用来实现抽屉背后的遮罩层。
 DrawerView: 主要类，负责抽屉的创建、布局、交互等。
 核心功能实现:
 布局: 使用 NSLayoutConstraint 来布局抽屉，并根据不同的状态调整位置。
 手势识别: 使用 UIPanGestureRecognizer 来处理拖动手势，根据手势的移动距离和速度来更新抽屉的位置。
 动画: 使用 UIViewPropertyAnimator 来实现抽屉的动画效果。
 状态管理: 通过 currentPosition 属性来跟踪抽屉当前的状态，并提供 setPosition 方法来设置抽屉的状态。
 子视图滚动: 可以禁用子视图的滚动，以便在拖动抽屉时不会发生冲突。
 覆盖层: 使用 Overlay 类来实现遮罩效果，可以自定义遮罩的颜色和透明度。
 代码亮点:

 模块化设计: 代码结构清晰，各个功能模块划分明确。
 可定制性高: 提供了丰富的属性和方法，可以根据需求定制抽屉的外观和行为。
 性能优化: 使用 UIViewPropertyAnimator 实现动画效果，性能较好。
 错误处理: 对一些特殊情况进行了处理，比如没有父视图、手势冲突等。
 潜在改进点:

 代码复杂度: 代码量较大，逻辑较为复杂，可以考虑进一步优化。
 文档注释: 可以添加更多的注释，提高代码的可读性。
