...
- (void)refreshUIWithModel:(BUNativeAd *_Nonnull)model {
    ...
    [KingfisherHelper setImageFor:self.nativeAdView.mediation.canvasView.imageView with:image.imageURL];
    ...
    if (model.data.mediation.adLogo.imageURL) {
        ...
        [KingfisherHelper setImageFor:adLogoV with:model.data.mediation.adLogo.imageURL];
...
- (void)refreshUIWithModel:(BUNativeAd *)model {
    ...
    if (image) {
        ...
        [KingfisherHelper setImageFor:self.nativeAdView.mediation.canvasView.imageView with:image.imageURL placeholder:image.mediation.image];
    }
...
- (void)refreshUIWithModel:(BUNativeAd *)model {
    ...
    [KingfisherHelper setImageFor:self.nativeAdView.mediation.canvasView.imageView with:model.data.imageAry[0].imageURL];

    
    if (model.data.imageAry.count > 1) {
        originX += (imageWidth + 5);
        self.iv2.frame = CGRectMake(originX, y, imageWidth, imageHeight);
        [KingfisherHelper setImageFor:self.iv2 with:model.data.imageAry[1].imageURL];
    }

    if (model.data.imageAry.count > 2) {
        originX += (imageWidth + 5);
        self.iv3.frame = CGRectMake(originX, y, imageWidth, imageHeight);
        [KingfisherHelper setImageFor:self.iv3 with:model.data.imageAry[2].imageURL];
    }
